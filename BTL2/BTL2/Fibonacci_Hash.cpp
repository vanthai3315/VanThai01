...
void Fibonacci_Hash::insert_n(int n, vector <pair<string, int>> kv)
{
    for (int i = 0; i < n; i++)
    {
        // Chèn vào bảng băm
...
int Fibonacci_Hash::getCollisionCount()
{
    return collisionCount;
}

// Đếm số cụm trong bảng băm
int Fibonacci_Hash::countClusters()
{
    int count = 0;
...
double Fibonacci_Hash::averageClusterSize()
{
...
int Fibonacci_Hash::maxClusterSize()
{
...
int Fibonacci_Hash::memory()
{
    return size * sizeof Entry;

}

double Fibonacci_Hash::averageSearchTime(const std::vector<std::pair<std::string, int>>& kv)
{
    int value;
    bool found;
...
double Fibonacci_Hash::averageRemoveTime(const std::vector<std::pair<std::string, int>>& kv)
{
    bool removed;
    double totalTime = 0.0;
